...
Task 1A creates `xtask/runner-lock-v1.json` only after building the locked release xtask. The sole capture/verify entry point is this closed five-row command; `runner capture-verify-matrix` dispatches one native probe to each named row, merges only those five signed capture records, re-verifies every row against the merged lock, and fails on a missing, duplicate, extra, or substituted runner:
...
No `xtask` command in this plan runs before the first command succeeds, and no runner id is supplied through a placeholder or free-form label. Each record contains the exact hardware identifiers, RAM bytes, OS image/product/build, kernel, display values, monitor/scale/refresh, GTK/WebKitGTK or WKWebView version, VM image digest when virtualized, and snapshot provider. Every scenario internally calls the same closed-set verifier against `xtask/runner-lock-v1.json`; a missing field, `+` version range, hardware substitution, display/session mismatch, image-digest mismatch, or reused snapshot name fails before spawn.
...
| `xtask/{Cargo.toml,src/main.rs,src/{fixtures,metrics,gui,package}.rs}` | Task 1A deterministic fixtures, GUI driver, metric assertions, packages |
...
| `spikes/feathermark-spike-support/src/{scheduler,transport,typed_render,scroll}.rs` | Task 1C spike-local executable implementations consuming Task-1A types/protocol; never owns `SafeLinkTarget` and is never linked by `feathermark-app` |
...
| `fuzz/{Cargo.toml,rust-toolchain.toml,fuzz_targets/*.rs,corpus/*}` | Task 1A standalone `nightly-2026-07-01` cargo-fuzz package and seed corpora |
...
- [ ] Create the standalone `fuzz/` package with `[workspace] members = []`, `rust-toolchain.toml` pinned to `nightly-2026-07-01`, seed corpus, and targets `render_markdown`, `source_blocks`, and `preview_event`. Each target asserts no panic/hang, valid UTF-8 boundaries, ordered/revisioned blocks, bounded output, and the generated-node allowlist. Smoke-build each exact target with `cargo +nightly-2026-07-01 fuzz build --fuzz-dir fuzz render_markdown`, then `source_blocks`, then `preview_event`.
...
- [ ] Stop immediately on any hard failure, missing runner, missing raw samples, native-Wayland fallback to XWayland, unsupported GTK/Wry lifecycle, both macOS candidates failing, or contradictory Ferrite evidence. Do not create `feathermark-app`.
