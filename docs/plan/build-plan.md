...
The five jobs upload exactly `rutile-package-fm-macos-arm64-v1`, `rutile-package-fm-macos-x86_64-v1`, `rutile-package-fm-ubuntu-x11-v1`, `rutile-package-fm-ubuntu-wayland-v1`, and `rutile-package-fm-fedora-wayland-v1`, each containing only its like-named `package-<runner>.tar.zst`. The package fan-in job downloads them into `target/evidence/package-fan-in/` and runs exactly one global assertion with five explicit, non-glob archives:
