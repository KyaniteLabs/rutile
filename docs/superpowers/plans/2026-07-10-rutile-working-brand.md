...
**Files:**
- Create: `crates/rutile-app/src/brand.rs`
- Modify: `crates/rutile-app/src/lib.rs`
- Test: `crates/rutile-app/src/brand.rs`
...
- [ ] Run `cargo test -p rutile-app --lib --locked` and verify the new tests fail because the brand contract is absent.
- [ ] Implement the constants and formatter, then export `pub mod brand` from `lib.rs`.
- [ ] Re-run `cargo test -p rutile-app --lib --locked` and verify it passes.

### Task 2: Native Shell Display Copy

**Files:**
- Modify: `crates/rutile-app/src/main.rs`
- Modify: `crates/rutile-app/src/platform/macos/native.rs`
- Modify: `crates/rutile-app/src/platform/linux_gtk.rs`
...
- [ ] Replace only user-visible `Rutile` literals with the brand contract; retain internal smoke filenames, environment variables, protocols, and bridge names.
- [ ] Run `cargo test -p rutile-app --all-targets --locked` and verify it passes.
...
- Preserves: `rutile` binary, bundle ID, package IDs, schemas, install paths, and RPM spec filename.
...
- [ ] Replace the README heading with `Rutile` and add the endorsement line plus a short note that internal identifiers remain `rutile` during the working-name phase.
- [ ] Run `cargo fmt --all -- --check`.
- [ ] Run `cargo test --workspace --all-targets --locked`.
- [ ] Run a focused `rg` audit over current application and packaging sources; classify remaining Rutile references as technical or historical.
- [ ] Run the macOS feature build check: `cargo check -p rutile-app --no-default-features --features macos-shell --locked`.
