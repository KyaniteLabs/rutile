...
| macOS artifacts | `FeatherMark-0.1.0-macos-arm64.app.zip` and `FeatherMark-0.1.0-macos-arm64.dmg`; ad-hoc signed, unnotarized, labeled `local-unnotarized-macos-arm64`. |
| Linux artifacts | `FeatherMark-0.1.0-linux-x86_64.tar.zst`, `.deb`, and `.rpm`; labeled `linux-x86_64-unverified-wayland`. The local-beta RPM manifest records `rpm_runtime_verified=false`; Fedora installation is a later promotion gate. |
...
|---|---|
| Wave 1 shared | `Cargo.toml`, `Cargo.lock`, `crates/feathermark-app/Cargo.toml`, `crates/feathermark-app/src/app.rs`, `crates/feathermark-app/src/main.rs`, `crates/feathermark-app/src/preview_host.rs`, `crates/feathermark-app/src/render_scheduler.rs`, `crates/feathermark-app/tests/app_reducer.rs`, `crates/feathermark-app/tests/preview_host.rs` |
| Wave 2M macOS | `crates/feathermark-app/src/platform/macos.rs`, `crates/feathermark-app/src/platform/macos/editor.rs`, `crates/feathermark-app/src/platform/macos/native.rs`, `crates/feathermark-app/tests/macos_product.rs` |
| Wave 2L Linux | `crates/feathermark-app/src/platform/linux_gtk.rs`, `crates/feathermark-app/tests/linux_product.rs`, `scripts/feathermark-linux-lifecycle.sh` |
...
1. `refactor: freeze FeatherMark shared product contracts`
2. `feat: complete FeatherMark macOS product shell`
3. `feat: complete FeatherMark Linux product shell`
4. `feat: add deterministic local FeatherMark packaging`
5. `release: package FeatherMark local beta artifacts`
...
- branch is `feat/feathermark-build`;
...
**Files:** `Cargo.toml`, `Cargo.lock`, `crates/feathermark-app/Cargo.toml`
...
cargo test --locked -p feathermark-app
```
...
**Files:** `crates/feathermark-app/src/app.rs`, `crates/feathermark-app/tests/app_reducer.rs`
...
```bash
cargo test --locked -p feathermark-app --test app_reducer
```
...
**Files:** `crates/feathermark-app/src/render_scheduler.rs` and its existing unit tests
...
```bash
cargo test --locked -p feathermark-app render_scheduler
```
...
**Files:** `crates/feathermark-app/src/preview_host.rs`, `crates/feathermark-app/tests/preview_host.rs`
...
```bash
cargo test --locked -p feathermark-app --test preview_host
```
...
**Files:** `crates/feathermark-app/src/main.rs`
...
```bash
cargo test --locked -p feathermark-app
cargo clippy --locked -p feathermark-app --all-targets -- -D warnings
cargo fmt --all -- --check
git diff --check
```
...
**Files:** `crates/feathermark-app/src/platform/macos/editor.rs`, `crates/feathermark-app/tests/macos_product.rs`
...
```bash
cargo test --locked -p feathermark-app --no-default-features --features macos-shell,test-control --test macos_product incremental
```
...
**Files:** `crates/feathermark-app/src/platform/macos/native.rs`, `crates/feathermark-app/src/platform/macos.rs`
...
**Files:** `crates/feathermark-app/src/platform/macos.rs`, `crates/feathermark-app/src/platform/macos/native.rs`, `crates/feathermark-app/tests/macos_product.rs`
...
**Files:** `crates/feathermark-app/src/platform/macos.rs`, `crates/feathermark-app/src/platform/macos/native.rs`
...
**Files:** `crates/feathermark-app/tests/linux_product.rs`, `scripts/feathermark-linux-lifecycle.sh`
...
**Files:** `crates/feathermark-app/src/platform/linux_gtk.rs`, `crates/feathermark-app/tests/linux_product.rs`, `scripts/feathermark-linux-lifecycle.sh`
...
**Files:** `crates/feathermark-app/src/platform/linux_gtk.rs`, `crates/feathermark-app/tests/linux_product.rs`
...
```text
macOS app staging: <output-root>/_staging/app/FeatherMark.app
macOS artifacts: <output-root>/FeatherMark-0.1.0-macos-arm64.app.zip and .dmg
Linux archive staging: <output-root>/_staging/archive/FeatherMark-linux-x86_64
Linux DEB staging: <output-root>/_staging/deb
Linux RPM topdir: <output-root>/_staging/rpm
Linux artifacts: <output-root>/FeatherMark-0.1.0-linux-x86_64.tar.zst, feathermark_0.1.0_amd64.deb, and feathermark-0.1.0-1.x86_64.rpm
```
...
4. Retain the direct `hdiutil create -volname FeatherMark -srcfolder ... -format UDZO` plan for the DMG.
...
3. Generate a Debian staging tree containing `/usr/bin/feathermark`, `/usr/share/doc/feathermark/package-manifest-v1.json`, and `DEBIAN/control` with the locked dependencies.
...
```text
target/local-release/macos-arm64/FeatherMark-0.1.0-macos-arm64.app.zip
target/local-release/macos-arm64/FeatherMark-0.1.0-macos-arm64.dmg
...
```text
target/local-release/linux-x86_64/FeatherMark-0.1.0-linux-x86_64.tar.zst
target/local-release/linux-x86_64/feathermark_0.1.0_amd64.deb
target/local-release/linux-x86_64/feathermark-0.1.0-1.x86_64.rpm
