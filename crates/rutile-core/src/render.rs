...
const PREVIEW_CSS: &str = "rutile://preview/v1/assets/preview.css";
const PREVIEW_BRIDGE: &str = "rutile://preview/v1/assets/bridge.js";
...
fn complete_page(revision: Revision, body: &str) -> String {
    format!(
        "<!doctype html><html data-rutile-revision=\"{revision}\"><head><meta charset=\"utf-8\"><meta http-equiv=\"Content-Security-Policy\" content=\"{CSP}\"><link rel=\"stylesheet\" href=\"{PREVIEW_CSS}\"><script defer src=\"{PREVIEW_BRIDGE}\"></script></head><body>{body}</body></html>"
    )
}
