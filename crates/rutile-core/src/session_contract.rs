...
//! and recovery loader live in `FileService`/`rutile-app`. Records
//! follow `rutile-protocol`'s versioning style: a `schema` tag plus a
...
/// `rutile-protocol`'s error taxonomy.
#[derive(Debug, Error)]
