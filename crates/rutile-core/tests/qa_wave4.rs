...
fn quote_toggle_over_edit_cap_is_a_typed_error_not_a_panic() {
    use rutile_core::{EditPlanError, MAX_PLAN_EDITS};
...
fn blockquote_amplification_is_bounded_not_hanging() {
    ...
    match result {
        Ok(out) => assert!(out.len() <= rutile_core::MAX_OUTPUT_BYTES),
...
fn append_journal_line(dir: &std::path::Path, line: &[u8]) {
    let journal = dir.join(rutile_core::AUTOSAVE_JOURNAL_FILE);
...
fn unsupported_version_journal_entry_is_skipped() {
    ...
    append_journal_line(
        &dir.0,
        format!(
            "{{\"schema\":\"rutile.autosave.v1\",\"v\":2,\"sequence\":9,\"captured_at_unix_ms\":9,\"document_path\":null,\"document_revision\":0,\"snapshot_file\":\"autosave-9.md\",\"snapshot_bytes\":1,\"snapshot_blake3\":\"{}\"}}\n",
            "0".repeat(64)
        )
        .as_bytes(),
    );
...
fn path_traversal_snapshot_reference_is_unrecoverable() {
    ...
    append_journal_line(
        &dir.0,
        format!(
            "{{\"schema\":\"rutile.autosave.v1\",\"v\":1,\"sequence\":5,\"captured_at_unix_ms\":5,\"document_path\":null,\"document_revision\":0,\"snapshot_file\":\"../../etc/passwd\",\"snapshot_bytes\":1,\"snapshot_blake3\":\"{}\"}}\n",
            "0".repeat(64)
        )
        .as_bytes(),
    );
