...
impl TestDir {
    fn new(name: &str) -> Self {
        let path = std::env::temp_dir().join(format!(
            "rutile-autosave-integration-{name}-{}-{}",
            std::process::id(),
...
fn snap(text: &str) -> rutile_core::DocumentSnapshot {
    Document::new(text).unwrap().snapshot()
}

fn lock_file_path(dir: &Path) -> PathBuf {
    dir.join(rutile_core::AUTOSAVE_LOCK_FILE)
}
...
fn all_corrupt_journal_returns_typed_rejected_reasons() {
    ...
    fs::write(
        dir.0.join(rutile_core::AUTOSAVE_JOURNAL_FILE),
        "not json\n{\"v\":2}\n",
    )
    .unwrap();
...
fn oversize_journal_is_rejected_pre_allocation() {
    ...
    fs::write(dir.0.join(rutile_core::AUTOSAVE_JOURNAL_FILE), &huge).unwrap();
...
fn oversize_session_state_is_rejected_pre_allocation() {
    ...
    fs::write(dir.0.join(rutile_core::SESSION_STATE_FILE), &huge).unwrap();
...
fn sequence_overflow_returns_sequence_exhausted() {
    ...
    let entry = format!(
        "{{\"schema\":\"rutile.autosave.v1\",\"v\":1,\"sequence\":{},\"captured_at_unix_ms\":1,\"document_path\":null,\"document_revision\":0,\"snapshot_file\":\"autosave-{}.md\",\"snapshot_bytes\":0,\"snapshot_blake3\":\"{}\"}}\n",
        u64::MAX,
        u64::MAX,
        "0".repeat(64)
    );
    fs::write(dir.0.join(rutile_core::AUTOSAVE_JOURNAL_FILE), entry).unwrap();
...
fn oversized_snapshot_is_rejected_with_typed_reason() {
    use rutile_core::RejectionReason;
    ...
    let big = vec![b'x'; rutile_core::MAX_DOCUMENT_BYTES + 1];
