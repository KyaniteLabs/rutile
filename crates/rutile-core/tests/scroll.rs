...
fn no_scroll_is_the_first_branch_in_both_directions() {
    for (source_max_top, preview_max_y) in [(0, 0.0), (0, 100.0), (100, 0.0)] {
        ...
        assert_eq!(map.source_to_preview(7, usize::MAX).unwrap(), 0.0);
        assert_eq!(map.preview_to_source(7, f64::NAN).unwrap(), 0);
    }
}
...
fn source_mapping_uses_containing_then_greatest_prior_block() {
    ...
    assert_eq!(map.source_to_preview(7, 5).unwrap(), 0.0);
    assert_eq!(map.source_to_preview(7, 15).unwrap(), 0.0);
    assert_eq!(map.source_to_preview(7, 20).unwrap(), 25.0);
    assert_eq!(map.source_to_preview(7, 69).unwrap(), 25.0);
}
...
fn positions_before_the_first_anchor_fall_back_to_ordinal_zero() {
    ...
    assert_eq!(map.source_to_preview(3, 0).unwrap(), 12.0);
    assert_eq!(map.preview_to_source(3, 0.0).unwrap(), 10);
}

#[test]
fn source_bottom_maps_exactly_to_preview_bottom() {
    let map = map(80, 100.0);
    assert_eq!(map.source_to_preview(7, 80).unwrap(), 100.0);
    assert_eq!(map.source_to_preview(7, usize::MAX).unwrap(), 100.0);
}

#[test]
fn preview_mapping_binary_searches_tops_and_breaks_ties_by_greater_ordinal() {
    let map = map(80, 100.0);
    assert_eq!(map.preview_to_source(7, 0.0).unwrap(), 0);
    assert_eq!(map.preview_to_source(7, 24.0).unwrap(), 40);
    assert_eq!(map.preview_to_source(7, 25.0).unwrap(), 40);
    assert_eq!(map.preview_to_source(7, 89.0).unwrap(), 70);
}

#[test]
fn preview_bottom_requests_document_eof() {
    let map = map(80, 100.0);
    assert_eq!(map.preview_to_source(7, 99.0).unwrap(), 100);
    assert_eq!(map.preview_to_source(7, 1000.0).unwrap(), 100);
}
...
fn empty_duplicate_gap_and_continuation_shaped_blocks_are_deterministic() {
    ...
    assert_eq!(map.source_to_preview(9, 0).unwrap(), 0.0);
    assert_eq!(map.source_to_preview(9, 15).unwrap(), 0.0);
    assert_eq!(map.source_to_preview(9, 35).unwrap(), 35.0);
    assert_eq!(map.preview_to_source(9, 0.0).unwrap(), 0);
}
...
fn user_gestures_own_fresh_interactions_and_target_the_other_pane() {
    let map = map(80, 100.0);
    let mut sync = ScrollSynchronizer::new(7, 41);
    let first = sync
        .handle_user(
            &map,
            Pane::Source,
...
fn stale_or_mismatched_user_events_do_not_consume_an_interaction_id() {
    let map = map(80, 100.0);
    let mut sync = ScrollSynchronizer::new(7, 5);
    assert!(matches!(
        sync.handle_user(
            &map,
            Pane::Source,
    ...
    assert_eq!(sync.next_interaction_id(), 6);
}
...
fn programmatic_echoes_and_reversals_never_emit_commands() {
    let map = map(80, 100.0);
    let mut sync = ScrollSynchronizer::new(7, 100);
...
fn one_hundred_alternating_gestures_have_unique_ids_and_zero_ping_pong() {
    let map = map(80, 100.0);
    let mut sync = ScrollSynchronizer::new(7, 1);
    for i in 0..100_u64 {
        let pane = if i % 2 == 0 {
            Pane::Source
        } else {
        ...
        assert_eq!(command.interaction_id, i + 1);
        assert!(matches!(
            sync.handle_programmatic(
                7,
                command.target_pane,
