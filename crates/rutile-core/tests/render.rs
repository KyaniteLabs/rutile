...
fn renders_a_revisioned_complete_page_with_only_fixed_assets() {
    ...
    assert!(
        rendered
            .page
            .contains("href=\"rutile://preview/v1/assets/preview.css\"")
    );
    assert!(
        rendered
            .page
            .contains("src=\"rutile://preview/v1/assets/bridge.js\"")
    );
...
fn links_use_the_shared_typed_policy_and_never_href() {
    ...
    assert!(
        rendered
            .body
            .contains("data-rutile-url=\"https://example.com/a\"")
    );
...
fn giant_inline_leaf_clones_formatting_and_link_shells_at_segment_cuts() {
    ...
    assert_eq!(
        rendered
            .body
            .matches("data-rutile-url=\"https://example.com/path\"")
            .count(),
        rendered.blocks.len()
    );
}
