...
use rutile_app::platform::linux_gtk::{
...
use rutile_app::preview_host::{PreviewControlSink, PreviewHost};
use rutile_core::{
...
fn product_session_edits_through_the_bounded_renderer_and_stages_preview() {
    let mut session = LinuxProductSession::new().unwrap();
    session.replace_all("# Rutile\n\nHello.", 10).unwrap();

    assert_eq!(session.revision(), 1);
    assert!(session.dirty());
    assert_eq!(session.undo(20).as_deref(), Some(STARTER_DOCUMENT));
    assert_eq!(session.redo(30).as_deref(), Some("# Rutile\n\nHello."));
    ...
    assert!(url.starts_with("rutile://preview/v1/document/3/"));
    assert_eq!(
        session
            .preview_host()
            .serve(&rutile_app::preview_host::SchemeRequest::get(url))
            .status,
        200
    );
}
...
fn scroll_sink_script_contains_only_the_typed_delivery() {
    ...
    impl PreviewControlSink for Capture {
        fn deliver_scroll_to(
            &mut self,
            delivery: rutile_app::preview_host::ScrollDelivery,
        ) -> Result<(), rutile_app::preview_host::HostError> {
