...
fn compile_fixture(name: &str, source: &str) -> std::process::Output {
    ...
    let directory = std::env::temp_dir().join(format!(
        "rutile-compile-contract-{}-{name}-{sequence}",
        std::process::id()
    ));
    ...
    let workspace_root = app
        .parent()
        .and_then(|p| p.parent())
        .expect("rutile-app lives under <workspace>/crates/");
...
fn render_execution_permit_is_not_cloneable() {
    let output = compile_fixture(
        ...
        r#"
use std::sync::Arc;
use rutile_app::render_scheduler::{RenderRequest, RenderScheduler, DEBOUNCE_MS};
fn main() {
