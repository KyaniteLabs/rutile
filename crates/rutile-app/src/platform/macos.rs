...
use rutile_protocol::{PreviewHostCommand, RenderUrl};
...
impl MacRenderWorker {
    fn new() -> Self {
        ...
        let thread = std::thread::Builder::new()
            .name("rutile-macos-render".to_owned())
            .spawn(move || {
...
impl ProductSession {
    ...
    pub(crate) fn accept_preview_event(
        &mut self,
        event: rutile_protocol::PreviewEventV1,
    ...
    pub fn render_now(&mut self) -> Result<RenderReceipt, MacError> {
        ...
        let page = match completion {
            ...
            Completion::PreviewTooLarge { revision } => {
                self.core.reduce(AppMessage::RenderFailed {
                    revision,
                    error: rutile_core::RenderError::PreviewTooLarge,
                });
        ...
        Ok(RenderReceipt {
            revision,
            page_bytes,
            url: format!("rutile://preview{}", url.document_path()),
        })
    }
