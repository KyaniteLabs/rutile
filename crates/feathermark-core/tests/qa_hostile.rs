...
fn assert_body_sanitized(body: &str, label: &str) {
    ...
    // Any data-feathermark-url must be a safe scheme (SafeLinkTarget invariant).
    for (idx, _) in body.match_indices("data-feathermark-url=\"") {
        let start = idx + "data-feathermark-url=\"".len();
        let val = &body[start..];
