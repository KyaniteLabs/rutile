...
use feathermark_app::preview_host::{
...
fn serves_only_exact_get_host_path_revision_and_nonce() {
    ...
    for request in [
        ...
        SchemeRequest::get("feathermark://other/v1/document/7/abababababababababababababababab"),
        SchemeRequest::get("feathermark://preview/v1/document/7/abababababababababababababababac"),
...
fn fixed_assets_and_document_have_exact_security_headers() {
    ...
    let css = host.serve(&SchemeRequest::get(
        "feathermark://preview/v1/assets/preview.css",
    ));
    let js = host.serve(&SchemeRequest::get(
        "feathermark://preview/v1/assets/bridge.js",
    ));
    ...
    assert!(bridge.contains("__feathermarkReceiveScrollTo"));
