...
# Rutile Linux WebKitGTK lifecycle runner.
#
# Runs the Rutile GTK binary for N cycles under an isolated D-Bus session
...
#   bash scripts/rutile-linux-lifecycle.sh --cycles 50 --display :1 --binary /path/to/rutile

# Re-exec under an isolated D-Bus session unless already isolated.
if [[ "${RUTILE_LIFECYCLE_ISOLATED:-}" != "1" ]]; then
  if ! command -v dbus-run-session >/dev/null 2>&1; then
    echo "dbus-run-session is required" >&2
    exit 2
  fi
  export RUTILE_LIFECYCLE_ISOLATED=1
...
if [[ -z "$binary" ]]; then
  binary="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/target/release/rutile"
fi
...
run_cycle() {
  ...
  RUTILE_LIFECYCLE_CYCLE="$cycle" \
  RUTILE_SMOKE_AUTOCLOSE_MS="$autoclose_ms" \
  ...
  if grep -q "RUTILE_ACTIVATION_FAILED" "$stderr_file"; then
