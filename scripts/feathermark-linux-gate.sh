...
# FeatherMark Linux product-shell gate.
#
...
cargo clippy --locked -p feathermark-app \
...
cargo build --locked -p feathermark-app --release \
  --no-default-features --features linux-gtk,test-control

echo "=== lifecycle smoke 50 cycles ==="
bash scripts/feathermark-linux-lifecycle.sh \
  --cycles 50 \
  --display ":${DISPLAY_NUM}" \
  --binary "${CARGO_TARGET_DIR}/release/feathermark"
